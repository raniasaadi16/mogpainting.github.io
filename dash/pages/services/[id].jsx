...
export default function Service({service}) {
    ...
    const [preview, setpreview] = useState(service.picture);
    ...
    const submit = async (e) => {
        ...
        service.serviceName !== data.serviceName && formData.append('serviceName', data.serviceName)
        service.picture !== data.picture && formData.append('picture', data.picture)
        service.description !== data.description && formData.append('description', data.description)
        service.color !== data.color && formData.append('color', data.color)
        service.icon !== data.icon && formData.append('icon', data.icon)
        console.log(formData)
        toggleLoading(true)
        try{
            const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_HOST}/api/services/${service._id}`, {
                method: 'PATCH',
                body: formData,
                headers:{
  ...
  return (
    <div>
      <p className="text-2xl font-bold mx-3">Update Service</p>
...
export async function getServerSideProps({params, req, res}){
    ...
    if(logData.data.isAuth){
        const resP = await fetch(`${process.env.NEXT_PUBLIC_SERVER_HOST}/api/services/${params.id}`)
        const dataP = await resP.json()
        return {
          props: {
            service: dataP.data.service
          }
        }
    }else{
        return {
